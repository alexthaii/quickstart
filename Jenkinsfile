node {
   ...
   stage('Build') {
        ...
        withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'artifactory', variable: 'USERPASS']]) {
            sh "sudo '${dockerHome}/bin/docker' login -u admin -p $USERPASS localhost:8081"
        }
